...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 -m gas_optimizer.verification.hevm_check <ContractName> [timeout] [keep_files]")
        print()
        print("Arguments:")
        ...
        print("  foundry/src/<ContractName>.sol          - Original contract")
        print("  foundry/src/<ContractName>Candidate.sol - Candidate contract")
        print()
        print("Example: python3 -m gas_optimizer.verification.hevm_check MyToken 120 true")
        sys.exit(1)

    # All paths below (src/..., forge) are relative to the Foundry workspace,
    # so run from there no matter where this script was invoked.
    os.chdir(Path(__file__).resolve().parents[2] / "foundry")
